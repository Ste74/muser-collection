...
echo "It reads/lists all pictures in selected directory and creates muser-collection-$DE.xml file in ~/.local/share/gnome-background-properties/."
...
sed "s|${DIRECTORY}|${ABSPATH}|g" muser-collection-$DE.xml > ~/.local/share/gnome-background-properties/muser-collection-$DE.xml

#Move again in Original path and clean all
mv ~/.local/share/gnome-background-properties/muser-collection-$DE.xml ${DIRECTORY}muser-collection-$DE.xml
rm lspictures.txt
echo
echo "#################################"
